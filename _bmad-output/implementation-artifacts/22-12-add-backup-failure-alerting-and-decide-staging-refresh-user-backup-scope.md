...
- [x] **AC1 (failure alerting):** The nightly backup workflow notifies an agreed channel when a non-best-effort step fails, verified by a controlled test. A single `if: failure()` step appended to the `backup-and-restore` job covers persistent CLI setup, required dumps/archive, upload, prune, download, and staging restore failures; explicitly tolerated `continue-on-error` steps do not trigger it. **Decided (owner, 2026-06-16): open/append a labelled `backup-failure` GitHub issue via the `gh` CLI using the built-in `GITHUB_TOKEN` — zero new secrets** (the step needs BOTH `permissions: issues: write` AND `env: GH_TOKEN: ${{ secrets.GITHUB_TOKEN }}`). **Do not reuse `src/lib/services/email-service.ts`**.
...
- [x] **AC3 (scope decision documented):** The staging-refresh and backup scope for `users`/auth identities is **explicitly decided and documented** in tracked `docs/commercial-readiness/09_operations_support_and_sla.md#backups-and-restore`. **Decided (owner-confirmed 2026-06-16): KEEP the `users` exclusion in the staging refresh** because `auth.users` is outside logical-backup scope and production `public.users.auth_user_id` values would not resolve in staging. Manual Auth re-provisioning/remapping is the accepted disaster-recovery step. Do **not** extend the refresh to `users`.
...
- [x] **Task 1 — Add backup-failure alerting** (AC: 1)
  - [x] Append a final step to the `backup-and-restore` job in `.github/workflows/supabase-nightly-backup.yml` guarded by `if: failure()` so it runs when any prior non-best-effort step fails. Place it after staging restore so it also captures a failed restore.
...
- [x] **Task 3 — Decide + document the `users`/auth + staging-refresh scope** (AC: 3)
  - [x] Apply the AC3 decision (owner-confirmed: **keep the `users` exclusion**) in tracked `docs/commercial-readiness/09_operations_support_and_sla.md#backups-and-restore`; local operator notes may supplement but cannot be the sole evidence.
  - [x] Affirm Auth re-provisioning and `public.users.auth_user_id` remapping as the accepted manual disaster-recovery step because `auth.users` is never in the logical dump.
  - [x] State explicitly that 22.11 removed the email-spam concern, but the refresh **still excludes `users`** to avoid desynchronizing production `public.users.auth_user_id` values from staging `auth.users`. Keep the `public.users`-in-full-dump vs `auth.users`-excluded distinction clear.
...
- Net: extending the refresh to `users` trades a now-removed email risk for a real **auth-link-integrity** risk. Keeping the exclusion is the safer default; manual Auth provisioning/remapping is the accepted recovery approach, recorded in tracked `docs/commercial-readiness/09_operations_support_and_sla.md#backups-and-restore`.
...
- The tracked AC3 source of truth is `docs/commercial-readiness/09_operations_support_and_sla.md#backups-and-restore`; any local operator notes are supplementary and cannot carry acceptance evidence alone.
...
- AC3 doc target: tracked `docs/commercial-readiness/09_operations_support_and_sla.md#backups-and-restore` — staging exclusion, full-backup/Auth boundary, and manual Auth re-provision/remap decision
...
- **AC1 — failure alerting (verified).** Added a final `if: failure()` step `Alert on backup failure (open/append a backup-failure issue)` to the `backup-and-restore` job; it fires after any failed step not explicitly marked `continue-on-error` and opens — or comments on an existing open — labelled `backup-failure` GitHub issue via the `gh` CLI + built-in `GITHUB_TOKEN`. **Zero new secrets.** Story 22.13 later made the runtime partial archive required, leaving only the roles dump best-effort.
...
- **AC3 — scope decision documented (kept exclusion).** Per owner decision, the staging refresh **keeps** the `users`/auth exclusion. The durable decision is in tracked `docs/commercial-readiness/09_operations_support_and_sla.md#backups-and-restore`: the full public dump contains `public.users`, but not managed `auth.users`; manual Auth re-provision/remap is the accepted disaster-recovery step.
...
Supplementary local operator notes were also updated during the original implementation, but the tracked `09_operations_support_and_sla.md#backups-and-restore` section is now canonical for AC3.

## Change Log

| Date | Change |
| --- | --- |
| 2026-06-16 | Story 22.12 context created (ready-for-dev): backup-failure alerting + one-shot CLI-setup retry on `supabase-nightly-backup.yml`; `users`/auth staging-refresh scope decision and manual Auth recovery step; AC4 readiness-surface sync (`09_`, `14_`, `R-007`/`R-019`, `00_index`); testable alert-message builder + controlled failure verification. |
...
| 2026-06-16 | Implemented (status → review). AC1 GitHub-issue alerting + AC2 CLI pin/retry; AC3 keep-`users` exclusion with auth-link-integrity rationale; readiness surfaces synced; 8 alert-builder tests and controlled workflow runs `27642547441`/`27642638386`; full applicable gates green; Playwright N/A. |
| 2026-07-09 | Story 22.13 moved the AC3 identity-scope decision into tracked `docs/commercial-readiness/09_operations_support_and_sla.md#backups-and-restore`, updated all acceptance-evidence references, and made the runtime partial archive required (only roles remains best-effort). |
...
1. **AC3 scope — KEEP the `users` exclusion in the staging refresh.** Confirmed. Do not extend the refresh to `users`; tracked `docs/commercial-readiness/09_operations_support_and_sla.md#backups-and-restore` records the auth-link-integrity rationale and manual Auth re-provision/remap step.
...
_Code review 2026-06-17 (Round 1) — adversarial layers: Blind Hunter, Edge Case Hunter, Acceptance Auditor. AC1–AC5 all verified MET against the spec; the items below are residual gaps, not AC failures. No Critical/High confirmed (the Edge Case Hunter's "High" was downgraded after verifying the headline missing-from-bucket path still alerts)._

_Outcome: 1 decision (resolved → keep dump tolerance, fix comment) + 4 patches applied 2026-06-17; gates re-run green (`node --check` OK, 8/8 unit tests pass, `tsc --noEmit` EXIT:0, `eslint scripts/notify-backup-failure.mjs` EXIT:0). 5 low-severity items deferred to `deferred-work.md`, 4 dismissed as noise. Story → `done`._

- [x] [Review][Patch] (fixed 2026-06-17; superseded in part by Story 22.13) Corrected the alert comment to cover failures not marked `continue-on-error`. At that review point roles and the partial dump were best-effort; Story 22.13 subsequently made the runtime partial archive required, so only roles remains intentionally tolerated.
- [x] [Review][Patch] (fixed 2026-06-17) Stale "add backup-failure alerting" language in three sibling readiness docs contradicts the four surfaces 22.12 updated [docs/commercial-readiness/01_executive_summary.md:61](docs/commercial-readiness/01_executive_summary.md#L61), [docs/commercial-readiness/08_security_overview.md:76](docs/commercial-readiness/08_security_overview.md#L76), [docs/commercial-readiness/12_commercial_pack.md:109](docs/commercial-readiness/12_commercial_pack.md#L109) (also `01_:49`, `08_:29`, `08_:60`) still list alerting as a pending/remaining action. AC4 named only `09_/14_/11_/00_` (so literal AC4 is MET), but the readiness package is now internally inconsistent. Fix: update those bullets to "implemented in Story 22.12," mirroring the `09_`/`11_` wording.
- [x] [Review][Patch] (fixed 2026-06-17) Alert "Failed step/job" is always the job id, never the failing step [.github/workflows/supabase-nightly-backup.yml:137](.github/workflows/supabase-nightly-backup.yml#L137) — `FAILED_STEP: ${{ github.job }}` passes `backup-and-restore` regardless of which step failed (confirmed in evidence issue #89). GitHub has no built-in failed-step expression; fix by relabelling the builder's line to "Failed job" so it does not promise step-level precision it cannot deliver.
- [x] [Review][Patch] (fixed 2026-06-17) Dedupe falls through to a duplicate issue when `gh issue list` fails [scripts/notify-backup-failure.mjs:650](scripts/notify-backup-failure.mjs) — `findOpenIssueNumber()` returns `null` on a non-zero `gh issue list` exit, indistinguishable from "no open issue," so a transient list failure proceeds to `gh issue create` and opens a second `backup-failure` issue, defeating the dedupe guarantee. Fix: distinguish list-failure from an empty result and do not open a new issue on list-failure.
- [x] [Review][Defer] Dedupe is check-then-act → concurrent failing runs can each open an issue [scripts/notify-backup-failure.mjs] — deferred, low likelihood with a single nightly cron; true fix needs locking.
- [x] [Review][Defer] `if: failure()` does not fire on job cancellation/timeout/runner eviction [.github/workflows/supabase-nightly-backup.yml:132](.github/workflows/supabase-nightly-backup.yml#L132) — deferred; `|| cancelled()` would add it but risks noise on manual cancels (owner call).
- [x] [Review][Defer] Side-effecting half of the script is untested (only the pure builder has unit coverage) [scripts/notify-backup-failure.mjs] — deferred; `findOpenIssueNumber`/open-vs-comment/`ensureLabel`/`resolveRunUrl`/exit codes covered only by the one-off `workflow_dispatch` run; mocking `spawnSync('gh')` is added scope.
- [x] [Review][Defer] Alert date from `new Date()` can skew vs the run's trigger date near UTC midnight; the documented `BACKUP_DATE_UTC` override is never set by the workflow (dead config) [scripts/notify-backup-failure.mjs] — deferred, cosmetic; run ID disambiguates.
- [x] [Review][Defer] The alert channel itself can fail silently (gh/token misconfig) with no secondary fallback [.github/workflows/supabase-nightly-backup.yml:131](.github/workflows/supabase-nightly-backup.yml#L131) — deferred; inherent to the owner-chosen single-channel / zero-new-secrets design.

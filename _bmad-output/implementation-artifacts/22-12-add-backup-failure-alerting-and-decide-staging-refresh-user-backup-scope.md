...
- [x] [Review][Patch] (fixed 2026-06-17) Correct the alert-step comment so it does not overclaim "dump" coverage (owner decision 2026-06-17: KEEP the `continue-on-error` tolerance on the two dump steps as deliberate — the roles dump can fail benignly on managed Postgres) — `Dump roles` [.github/workflows/supabase-nightly-backup.yml:74](.github/workflows/supabase-nightly-backup.yml#L74) and `Dump employees and column_config` [:87](.github/workflows/supabase-nightly-backup.yml#L87) carry `continue-on-error: true`, so a failure there does NOT trip `if: failure()`; yet the alert-step comment [:123-128](.github/workflows/supabase-nightly-backup.yml#L123) claims it "Fires when ANY prior step in this job fails (CLI setup, **dump**, …)". Fix: reword the comment to state it fires on any prior step that is **not** `continue-on-error` (i.e. schema/data dump, upload, prune, download, staging restore, and persistent CLI-setup failure), and that the two best-effort dump steps are intentionally tolerated. The behavior (keep tolerance) stays; only the comment changes.

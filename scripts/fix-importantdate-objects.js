...
function processFile(filePath) {
  ...
  const importantDatePattern =
    /(week_number:\s*(?:number|null|\d+),?\s*\n\s+year:\s*\d+,?\s*\n\s+category:\s*['"`][^'"`]+['"`],?\s*\n\s+date_description:[\s\S]*?updated_at:\s*['"`][^'"`]+['"`],?\s*\n\s+)(})/g;
...
console.log(
  '🔧 Fixing ImportantDate objects with wrong Employee properties...\n'
);

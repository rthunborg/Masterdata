...
const filePath = path.join(__dirname, 'tests/unit/components/employee-table.test.tsx');
...
const pattern = /(\s+created_at: "[^"]+",\s+db_column_name: '[^']+',\s+category_color: '#[A-F0-9]+',\s+display_order: \d+,\s+)is_visible: true,(\s+updated_at: [^\n]+)/g;
...
if (fixedContent !== content) {
  ...
  console.log(`✅ Fixed ${matchCount} duplicate is_visible properties in ColumnConfig objects`);
} else {

 ...
 * Used by createEmployeeSchemaWithMessages / updateEmployeeSchemaWithMessages
 */
...
const englishMessages: Record<keyof typeof validationMessages, string> = {
  firstNameRequired: "First name is required",
  firstNameMaxLength: "First name must be less than 100 characters",
  surnameRequired: "Surname is required",
  surnameMaxLength: "Surname must be less than 100 characters",
  ssnRequired: "SSN is required",
  ssnFormat: "SSN must be in format YYYYMMDD-XXXX or YYMMDD-XXXX",
  emailRequired: "Email is required",
  emailInvalid: "Invalid email format",
  rankRequired: "Rank is required",
  rankInvalid: "Rank must be SEV or CHEF",
  genderInvalid: "Gender must be Man or Woman",
  hireDateRequired: "Hire date is required",
  hireDateInvalid: "Invalid date format",
  hireDateFuture: "Hire date cannot be in the future",
  stenaDateRequired: "Stena date is required",
  omcDateRequired: "OMC date is required",
  terminationDateRequired: "Termination date is required",
  terminationDateInvalid: "Invalid date format",
  terminationReasonRequired: "Termination reason is required",
  terminationReasonMaxLength: "Termination reason must be 500 characters or less",
  updateFieldRequired: "At least one field must be provided for update",
  dietDetailsRequired: "Diet details are required when special diet is selected",
};
...
/**
 * Single source of truth for employee field definitions.
 * Used by create, update, and translated schema variants.
 * When called without a translation function, uses English defaults.
 */
function getBaseEmployeeSchemaObject(t?: (key: string) => string) {
  const msg = (key: keyof typeof validationMessages) =>
    t ? t(validationMessages[key]) : englishMessages[key];
  ...
  return z.object({
    ...
    one: z.boolean(),
    one_marked_at: z.string().datetime().nullable().optional(),
    talmundo: z.boolean(),
    isps: z.boolean(),
    photo: z.boolean(),
    origo: z.boolean(),
    ...
    repayment_needed_omc: z.union([z.boolean(), z.null()]).optional(),
    repayment_needed_pe3: z.union([z.boolean(), z.null()]).optional(),
...
/**
 * Base schema with English defaults — single source of truth for field definitions.
 * createEmployeeSchema, updateEmployeeSchema, and their WithMessages variants
 * all derive from getBaseEmployeeSchemaObject.
 */
...
export const csvImportEmployeeSchema = z.object({
  ...
  // Story 8.13: Repayment tracking fields - boolean checkbox; coerce string for CSV import
  repayment_needed_omc: z.union([
    z.boolean(),
    z.string().transform((v) => (v === '' || v === 'false' || v === '0' ? null : true)),
    z.null(),
  ]).nullable().default(null).optional(),
  repayment_needed_pe3: z.union([
    z.boolean(),
    z.string().transform((v) => (v === '' || v === 'false' || v === '0' ? null : true)),
    z.null(),
  ]).nullable().default(null).optional(),

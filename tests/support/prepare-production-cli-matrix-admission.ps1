...
try {
  ...
  $outputDirectory = Require-AbsolutePath $outputDirectory 'matrix_admission_input'
  ...
  $resources = @($listed.data.resources | Where-Object { $_.resourceId -eq $resourceId })
  Need ($resources.Count -eq 1) 'matrix_admission_guard_resource'
  ...
  Need ($inspectExitCode -eq 0 -and $mappings.Count -eq 1 -and [string]$mappings[0].HostIp -match '^(127\.0\.0\.1|::1)$' -and [string]$mappings[0].HostPort -eq [string]$port) 'matrix_admission_container'
  ...
  Need ($LASTEXITCODE -eq 0 -and $system -match '^[1-9][0-9]{0,18}$') 'matrix_admission_identity'
  $sha = [Security.Cryptography.SHA256]::Create()
  try { $systemHash = ([BitConverter]::ToString($sha.ComputeHash([Text.Encoding]::UTF8.GetBytes($system)))).Replace('-','').ToLowerInvariant() } finally { $sha.Dispose(); $system = $null }

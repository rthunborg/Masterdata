...
const manifest = JSON.parse(
  readFileSync(
    resolve(root, 'supabase/migration-baseline-manifest.json'),
    'utf8'
  )
) as Manifest;
...
describe('Story 22.15 migration baseline safety', () => {
  ...
  it('restores the original immutable room-assignment migration byte-for-byte', () => {
    const digest = createHash('sha256')
      .update(
        readFileSync(
          resolve(migrationDir, '20250113000000_add_room_assignment_rpc.sql')
        )
      )
      .digest('hex');
    ...
    const expected: PolicyContract[] = [
      {
        ...
        roles: '{authenticated}',
        command: 'ALL',
        qual: "((selectget_user_role()asget_user_role)=any(array['hr_admin'::text,'recruiter'::text]))",
        withCheck: null,
      },
      {
        table: 'employees',
        ...
        roles: '{authenticated}',
        command: 'SELECT',
        qual: "(((selectget_user_role()asget_user_role)=any(array['sodexo'::text,'omc'::text,'payroll'::text,'toplux'::text,'crewing'::text]))and(is_archived=false))",
        withCheck: null,
      },
      {
        table: 'column_config',
      ...
      },
      {
        ...
        command: 'INSERT',
        qual: null,
        withCheck: "((selectget_user_role()asget_user_role)='hr_admin'::text)",
      },
      {
        table: 'users',
        ...
        roles: '{authenticated}',
        command: 'SELECT',
        qual: "(((selectget_user_role()asget_user_role)='hr_admin'::text)or(auth_user_id=(selectauth.uid()asuid)))",
        withCheck: null,
      },
      {
      ...
      },
      {
        ...
        roles: '{authenticated}',
        command: 'UPDATE',
        qual: "((selectget_user_role()asget_user_role)=any(array['hr_admin'::text,'crewing'::text]))",
        withCheck: null,
      },
      {
      ...
      },
      {
        ...
        roles: '{authenticated}',
        command: 'SELECT',
        qual: '(((selectauth.uid()asuid)=user_id)and((selectget_user_role()asget_user_role)isnotnull))',
        withCheck: null,
      },
      {
        table: 'user_filters',
  ...
  it('pins exact staffing headcount and saved-filter catalog contracts', () => {
    ...
    const staffingEnd = verifierSql.indexOf(
      "'dietary_columns_and_permissions'"
    );
    const userFiltersStart = verifierSql.indexOf("'user_filters_objects'");
    const userFiltersEnd = verifierSql.indexOf(
      "'represented_column_contracts'"
    );
  ...
  it('forces LF normalization for migration and verifier SQL', () => {
    expect(gitAttributes).toContain('supabase/migrations/*.sql text eol=lf');
  ...
  it('requires the pinned CLI, a signed per-version proof ledger, and an exact dry run', () => {
    ...
    expect(cutoverRunbook).toContain(
      'rejects every leading root option before command classification'
    );
    expect(cutoverRunbook).toContain(
      'accepts only the exact database argument shapes printed in this runbook'
    );
    ...
    expect(cutoverRunbook).toContain('EXPECTED_SUPABASE_SSL_ROOT_CERT_SHA256');
    expect(cutoverRunbook).toContain('sslmode=verify-full');
    expect(cutoverRunbook).toContain('A bare command resolved from `PATH`');
    expect(cutoverRunbook).toContain(
    ...
    expect(cutoverRunbook).toContain('Disable the Supabase Data API');
    expect(cutoverRunbook).toContain(
...
describe('Story 22.15 active authorization migration', () => {
  ...
  it('performs invariant checks and app-row deletion in one caller-bound transaction', () => {
    ...
    const deletion = migrationSql.indexOf(
  ...
  it('creates a restricted minimal cleanup outbox with retry-safe app-only tombstones', () => {
    ...
    const targetLookup = migrationSql.indexOf('FROM public.users', retryLookup);

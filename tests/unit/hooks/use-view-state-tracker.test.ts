...
import type { FilterState } from "@/lib/types/notifications";
...
describe("useViewStateTracker", () => {
  ...
  it("updates sort state when sort prop changes", () => {
    const employees = [createEmployee("1")];
    const initialSort: { column: string; direction: "asc" | "desc" } = { column: "first_name", direction: "asc" };

    const { result, rerender } = renderHook(
      ({ sort }: { sort: { column: string; direction: "asc" | "desc" } }) =>
        useViewStateTracker({
          employees,
          filters: {},
          sort,
    ...
    // Update sort
    const updatedSort: { column: string; direction: "asc" | "desc" } = { column: "surname", direction: "desc" };
  ...
  it("updates all state simultaneously when all props change", () => {
    ...
    const sort1: { column: string; direction: "asc" | "desc" } = { column: "first_name", direction: "asc" };

    const { result, rerender } = renderHook(
      ({ employees, filters, sort }: { employees: Employee[]; filters: FilterState; sort: { column: string; direction: "asc" | "desc" } }) =>
    ...
    const sort2: { column: string; direction: "asc" | "desc" } = { column: "surname", direction: "desc" };

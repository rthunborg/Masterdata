...
describe("TimePicker Component", () => {
  describe("Rendering", () => {
    ...
    it("renders with valid time value", () => {
      const onChange = vi.fn();
      const { container } = render(<TimePicker value="14:30" onChange={onChange} />);
    ...
    it("renders in disabled state", () => {
      const onChange = vi.fn();
      const { container } = render(<TimePicker value="14:30" onChange={onChange} disabled />);
  ...
  describe("User Interactions", () => {
    it("calls onChange with formatted time on valid input", () => {
      const onChange = vi.fn();
      const { container } = render(<TimePicker value={null} onChange={onChange} />);
  ...
  describe("Value Synchronization", () => {
    it("updates local value when prop value changes", () => {
      const onChange = vi.fn();
      const { container, rerender } = render(<TimePicker value="14:30" onChange={onChange} />);

      let input = getTimeInput(container);
      expect(input.value).toBe("14:30");

      // Change prop value
      rerender(<TimePicker value="09:00" onChange={onChange} />);

      input = getTimeInput(container);

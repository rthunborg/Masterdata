...
// Count issues before
let beforeDuplicates = (content.match(/crewing_done: null,\s+stena_date: null,\s+omc_date: null,\s+pe3_date: null,/g) || []).length;
...
// Count issues after
let afterDuplicates = (content.match(/crewing_done: null,\s+stena_date: null,\s+omc_date: null,\s+pe3_date: null,/g) || []).length;
let afterMissing = (content.match(/crewing_done: null,\s+\}/g) || []).length;

fs.writeFileSync(filePath, content, 'utf8');
console.log('✓ Fixed remaining employees.test.ts issues:');
console.log(`  - Removed ${beforeDuplicates - afterDuplicates} sets of duplicate properties`);
console.log(`  - Added ${beforeMissing - afterMissing} missing comments properties`);

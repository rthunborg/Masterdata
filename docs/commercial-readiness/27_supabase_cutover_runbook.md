...
> **2026-09-24 read-only stop condition.** A source-pinned production inventory observes a `pg_cron launcher` and a semver-named `pg_net` worker, while neither extension is installed in the connected database. The candidate classifier's zero name-pattern remainder does not satisfy the extension-qualified backend contract. These may be cluster-preloaded workers or may target another database; their active write/outbound capability has not been proved absent. Resolve their configuration, database scope and effective write/outbound paths with bounded read-only evidence before seeking the isolation-setting approval. If a path remains active, include its reversible shutdown and denial proof in the separately reviewed isolation change set. Do not treat zero current subscription/slot counts, Data API disablement or network restrictions as a substitute. [Fresh redacted observation](evidence/production-readonly-cutover-refresh-2026-09-24.md).

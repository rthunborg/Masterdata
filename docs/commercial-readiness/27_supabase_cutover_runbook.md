...
> **Current production preflight — 2026-09-20.** PR #108's pending-review snapshot is historical: reviewed `cce6e2e` merged as staging `fa681411` with an identical tree; main remains `8223509`. The current candidate's saved-filter collector is bounded, count-only, and read-only; it is not integrated into protected bootstrap admission and cannot authorize cleanup, isolation, history repair, or forward apply. A legacy-main nightly backup workflow can truncate/reload staging. Owner approval to disable the entire workflow is pending; after quiescence, obtain fresh staging preservation proof before relying on staging data evidence. Production stays paused/no-go: seven dated strict catalog failures, 55 UNPROVED unsigned ledger rows, 13 proposed forward executes, full technical isolation, cleanup, repair, main, deployment, and reopening gates remain. [Evidence placeholder](evidence/production-preflight-and-job-pause-2026-09-20.md).

...
Historical PR #95 was owner-authorized and merged into staging at `8c82bd8f4cc3c5076b2b6a37f4ced209bd8cba1c`. PR #96 was subsequently merged to staging at `3724809065f9e4559802fdcd80e7e526172cc8fd`; main remains `822350986f4c023948a7bbf490ddffc371185c4a`. A fresh fetch found no intervening commit and the tree equalled the reviewed PR #96 staging head before the new candidate work. The staging branch is the production candidate; staging is not required to match the older production database.

The unmodified staging catalog gate historically failed four groups despite 57 matching history rows. Reviewed forward migration `20260909115242_reconcile_saved_filters_and_room_acl.sql` resolves the saved-filter FK, nonempty-name check, standalone indexes, default and object names, and redundant room ACL grants. The exact existing dietary `admin_limited` read-only extension is preserved. The post-merge diagnostic then found two absent repayment defaults only; planned forward `20260910094517_reconcile_repayment_defaults.sql` sets both defaults to `false` without changing existing values. The manifest now defines **one staging repair, then seven forward applies**; the production candidate plan is **57 proven repairs, then eight applies**, subject to a separate fresh production inventory. Local verification of the new version is pending. A production mismatch requires another reviewed plan; no production proof or write is implied.

...
> **Current post-merge staging gate — 2026-09-24.** Reviewed PR #115 merged as staging `3fb3d72a3a26672d91be1a98a51e964b804fdd02` with an identical tree; later PR #116 changed documentation only. Clean, target-bound read-only proof reconfirmed 68/68 history, strict `post_apply` catalog 16/16, scoped advisors and unchanged preservation baselines at the PR #115 merge. The owner has verified the staging application. All technical production gates remain open; a fresh read-only Vercel check still shows the recorded static production pause target and zero active cron definitions. [Redacted staging proof](evidence/pr115-postmerge-staging-readonly-proof-2026-09-23.md); [owner input and pause proof](evidence/production-owner-cutover-input-2026-09-24.md); [production gate order](evidence/production-next-gates-2026-09-23.md).
...
Status: **Staging v68 is complete under the reviewed plan: history is 68/68 with no pending/remote-only version; all five post-apply phases passed, including strict catalog 16/16.** The latest proof source is the 2026-09-23 PR #115 merge `3fb3d72a3a26672d91be1a98a51e964b804fdd02`; clean post-merge read-only proof reconfirmed history, catalog, advisors, aggregates, permission hashes and audit preservation. The 2026-09-17 PR #104 proof remains dated historical evidence. The historical apply procedure below is fulfilled and must not be replayed. Owner application verification completed on 2026-09-24. Production remains paused and untouched by this staging proof.
...
5. For production only, retain the mandatory fresh-backup gate before the first hosted change: record the owner's local database/schema backup and verified-restore attestation, confirm that the backup was captured after the last production write, and confirm that the restore check used that same backup. If either fact cannot be confirmed, stop and obtain a newly captured, restore-verified backup under a separately approved method. A backup workflow dispatch needs its own authorization. Then obtain the separate traffic-isolation and hosted-setting authorizations and prove the isolation gate below before any history-repair write. Keep isolation active across every repair, forward migration and database post-apply verification. Retain the production pause and run application smoke against staging. Any later production application deployment requires its own reviewed isolation design and explicit deployment authorization.
...
8. Re-run the target-binding check, run the production `db push --reviewed-target --dry-run --skip-vault` shape through the reviewed CLI wrapper, and compare the exact ordered output with the proposed list. `--reviewed-target` is consumed by the wrapper and is never passed to the Supabase CLI. This dry run does not enable a production write.
9. The current reviewed wrapper intentionally rejects every production non-dry-run `db push` before executable, target, or TLS access. Do not bypass that refusal or add an alternate production apply command to this procedure. The standing migration-apply authorization does not override this technical block or any other production gate.
10. A future reviewed change may define a production apply procedure only after the exact per-function pre-execute proof, full isolation evidence, and all separate production gates have been reviewed. Until then, retain read-only proof, the dry-run comparison, and the production pause.
...
Every catalog-verifier row must return `passed = true` for the completed staging and final production `post_apply` gates. The current production `production_pre_apply` diagnostic instead fails seven named rows; require the exact observed-profile prerequisite in section B without relabeling those failures as passes. The automated verifier is a necessary fail-fast check for the known unsafe-replay surfaces; it is **not sufficient by itself** to approve all 55 production history repairs. Before the first production repair, prepare a 55-row proof ledger copied from the manifest. For every version, record the migration-defined objects/data effects, the fresh read-only catalog query or inventory evidence proving them materially present, result, reviewer, and timestamp. The technical owner must sign the complete ledger before any repair command runs. An unproved row halts the whole repair batch.
...
The completed 66/66 correction is historical. The forward-only `20260910184841_reconcile_column_config_timestamp_and_audit_trigger.sql` was applied once on staging at `2026-09-12T11:50:57.715Z` without row cleanup or history repair. The historical approval record retained staging repair `[]` and its one-file execute plan as the represented-baseline record; the current manifest separately retains the subsequently completed v68 plan recorded below. The retained dry-run list below documents exactly what was executed and must never be replayed. Its completed historical dry run listed this one apply:
...
## B. Production — 55 repair candidates plus thirteen executes

> **Current classification, 2026-09-16.** Production has exactly `headcount_need >= 0`, with no upper-bound conjunct supplied by immutable `20260314000002_add_headcount_upper_bound.sql`; the recorded aggregate has zero out-of-range values. That lower-only state is the existing narrow pre-apply exception. It follows immutable `20260314000001_add_update_staffing_need_rpc.sql`, whose required definer effect is absent because the observed staffing RPC is invoker. Both immutable migrations must execute and must not be repaired as applied. The staffing reclassification creates **no** accepted catalog state: the strict catalog remains unchanged and no-go pending a reviewed exact function profile. No production history repair or execute may start until the reviewed manifest, runner, regression tests, and [55-row effect ledger](evidence/production-repair-proof-ledger-2026-09-16.md) agree on this order. The ledger is a review template with 55 UNPROVED rows and zero signatures; the [redacted local-gate receipt](evidence/production-staffing-local-gates-2026-09-16-redacted.json) does not substitute for hosted proof. See [the staffing classification evidence](evidence/production-staffing-classification-2026-09-16.md).

Do not begin until all of Epic 22 is merged to staging, the owner verifies staging, the immutable candidate is reviewed and promotion-ready, and the owner records the controlled cutover start and rollback ownership. The owner verified staging and waived calendar maintenance-window coordination while production is unused. The owner has attested to local backups and a verified restore without supplying a folder path. Before the first hosted mutation, confirm that the backup was captured after the last production write and that the restore verification used that same backup. If either fact is uncertain or false, stop until a newly captured backup has been restore-verified under a separately approved method. The backup folder path is not required in repository evidence.

The owner subsequently confirmed that the restore-tested backup is newer than the last production write and separately approved proceeding with temporary isolation work. This is an owner attestation, not machine inspection of the backup. Immediately before the first hosted change, reconfirm freshness and record the cutover start; the approval does not cover filter cleanup, history repair, main merge, deployment, or reopening. The 2026-09-24 operator preflight found matching independent IPv4 egress observations but no usable IPv6 path. The candidate IPv4 address is held in encrypted private inputs. The isolation assessor accepts an IPv4-operator/IPv6-deny-all profile only with an empty IPv6 allowance, an explicit unavailable-IPv6 observation, and independent non-operator denial probes for both families. Neither a current IPv4 lookup nor the assessor alone proves isolation.

### Mandatory production database-window traffic-isolation and hosted-setting gate

> **2026-09-24 read-only worker classification.** A source-pinned production inventory observes a `pg_cron launcher` and a semver-named `pg_net` worker. A second bounded probe confirms both libraries are preloaded and configured for the connected database, but neither extension nor its usual job/request table is installed there; the reviewed 68-file migration chain does not create them. `cron.launch_active_jobs` remains on. This supports only a current-state inference that the observed workers have no normal job/request substrate. The candidate classifier's zero name-pattern remainder alone never satisfied the extension-qualified backend contract, and the installed binaries have not been hash-bound to upstream source. Recheck the exact extension/table/configuration state before the first hosted mutation and after each relevant schema or setting change; any new job/request substrate or changed worker scope is a stop condition requiring an explicit reversible shutdown and denial proof in the separately approved isolation change set. Zero current subscription/slot counts, Data API disablement and network restrictions do not substitute for this check. [Redacted observation and receipt identities](evidence/production-readonly-cutover-refresh-2026-09-24.md).

Migration `20260314000001` introduces the legacy definer staffing implementation, and migration `20260709194903` later replaces it with the caller-bound security implementation. Migration `20260614000000` also temporarily grants legacy execution privileges to `authenticated`/`service_role`, which `20260709194903` later revokes or narrows. Because each file commits independently and the thirteen-file push can stop between them, the production system must remain fully isolated throughout this transient legacy-definer interval. The fact that seasonal users have paused activity is useful scheduling context, but it is not technical isolation.
...
4. Restrict direct PostgreSQL and pooler ingress to the operator's privately reverified approved egress using [Network Restrictions](https://supabase.com/docs/guides/platform/network-restrictions). On an IPv4-only operator path, allow only its exact `/32` and deny all IPv6 ingress; do not invent an IPv6 operator address. Prove the applied lists, operator database connection, and independent non-operator IPv4 and IPv6 denial. Supabase network restrictions do not cover HTTPS APIs, which is why the Data API, Realtime, and application/client controls are separate mandatory gates.
...
If isolation cannot be proven, the production cutover is NO-GO. The proposed future order is: **fresh production backup -> publication/connection inventory -> technical traffic and Realtime isolation with session drain -> separately approved single-transaction cleanup of the exact 48 orphan filters and independent post-cleanup preservation proof -> individually proved and separately approved 55-version history repair with immediate reconciliation -> exact thirteen-version include-all dry run -> proposed thirteen-file database apply -> post-apply catalog/grant/direct-role/advisor/data verification -> retain the existing production pause -> restore only separately authorized prior Data API state -> bounded direct-role/database verification and staging application smoke -> restore only separately authorized database/Realtime controls while retaining the production pause and job shutdown -> verify restored publication, connection, access, and monitoring state**. Cleanup is one narrowly scoped batch, not 48 separate decisions. The current wrapper intentionally makes the proposed database-apply step unavailable; this order is not an executable production procedure. Keep both the Data API and Realtime service disabled through every future migration and all database post-apply verification. Do not merge/deploy early merely because the database step has begun. If a future push or any database verification fails, remain fully isolated and follow the failure procedure; restoration or continuation is an explicit owner decision based on the observed state.
...
### Explicit repair-after-observed-profile-proof list

First bind the read-only database connection to the linked production project and run the production-specific strict catalog diagnostic:

```bash
node supabase/verify/verify-production-baseline-catalog.mjs production_pre_apply
```

The 2026-09-23 observation fails exactly seven of 16 strict catalog groups: `dietary_columns_and_permissions`, `represented_column_contracts`, `represented_function_contracts`, `represented_policy_contracts`, `represented_trigger_contracts`, `user_filters_objects`, and `user_filters_trigger_function_contract`. This is a **failed** catalog gate, not an accepted variant or permission to ignore its output. The matching schema-only and data aggregates are a separate, bounded observed-state profile. Require a fresh protected read-only collection from the reviewed source, approved target and TLS, with the same exact seven failures and nine successes, all 14 schema groups, and the complete preservation aggregates matching the reviewed baseline. Any added failure, changed group, missing object, changed data aggregate or target/source mismatch stops the batch. The observed profile only establishes the input state for a reviewed forward reconciliation; it does not prove historical material effects or authorize a write.

Only after this exact observed-state prerequisite passes, the fresh production inventory proves every listed version's material effect or reviewed successor disposition, the complete 55-row ledger is individually evidenced and signed, and the owner separately authorizes this exact **history-repair** batch may the operator run:

The 2026-06-11 restore-drill record names `staffing_needs.target_headcount`, while the repository contract is `staffing_needs.headcount_need`. Treat that as a historical evidence contradiction, not as proof of either shape. Fresh read-only observation of `headcount_need >= 0` identifies the column only; its missing upper bound is assigned to the second forward apply `20260314000002_add_headcount_upper_bound.sql`, after `20260314000001_add_update_staffing_need_rpc.sql` and after zero out-of-range values are proved. It does not prove the remaining staffing tables, policies, grants, function contracts, seed/history effects, or any other staffing repair candidate. A `target_headcount` observation remains a stop condition requiring an approved forward reconciliation plan; do not repair staffing versions as represented without their individual signed proofs.
...
### Proposed production forward ordering (not currently executable)

After the 55 repairs, the dry run must list exactly these thirteen versions:

1. `20260314000001_add_update_staffing_need_rpc.sql`
2. `20260314000002_add_headcount_upper_bound.sql`
3. `20260614000000_reconcile_environments_security_and_policies.sql`
4. `20260615000000_add_is_checklist_item_to_column_config.sql`
5. `20260709194903_remediate_pr_91_security_findings.sql`
6. `20260710144000_atomic_external_column_presentation.sql`
7. `20260710150000_atomic_user_status_transition.sql`
8. `20260831200026_enforce_active_authorization_and_atomic_user_deletion.sql`
9. `20260909115242_reconcile_saved_filters_and_room_acl.sql`
10. `20260910094517_reconcile_repayment_defaults.sql`
11. `20260910115024_reconcile_post_apply_acl_and_policy_initplans.sql`
12. `20260910184840_reconcile_canonical_trigger_acl_prerequisite.sql`
13. `20260910184841_reconcile_column_config_timestamp_and_audit_trigger.sql`
...
Stop unless the dry-run output is exact. `--include-all` is required because the first pending version is older than the repaired history candidates; the reviewed runner accepts this production dry-run shape, while the operator records and verifies the exact ordered thirteen-version output. This list is a proposed forward ordering, not an executable production apply: the current wrapper intentionally rejects a non-dry-run production `db push` before executable, target, or TLS access. Do not bypass the reviewed wrapper, mark an older version applied, or infer that standing migration authorization makes this write available. For any future reviewed procedure: Record the full immutable commit SHA, ordered thirteen-version file set and SQL hashes, owner-confirmed fresh database/schema dumps, maintenance-window evidence, proven isolation evidence, and the unchanged production pause. The dumps are confirmed as containing the real data; no location, content, or independent restore-test claim belongs in this record.

No production apply command is provided by this runbook. A future reviewed revision must first establish the exact staffing-function proof and all other production gates before it can define an executable write procedure.

If a future reviewed procedure defines and completes that write, its expected database end state before any later deployment is 68 versions in sync / 17 policies, with all 16 strict post-apply catalog checks passing. While full database isolation remains active, it must repeat all database verification, advisors, repayment aggregates, permission hashes, affected-table publication inventory, and redacted evidence capture. `node supabase/verify/verify-production-baseline-catalog.mjs post_apply` must exit `0`. Retain the existing production pause. Restore database settings only under separate explicit approval, then verify direct-role/database behavior and run application smoke against the exact staging candidate. A production application deployment is blocked by the committed pause lock and requires a later separately reviewed operator-isolation design plus explicit authorization. No deployment or reopening follows from a dry run or a future database gate.

## Rollback and failure behavior

- `migration repair` records history only and requires its own explicit owner authorization. Before repair, any deviation from the exact reviewed seven-failure observed profile, any missing material-effect proof, or any cleanup/preservation mismatch causes a stop with no repair write. The seven documented failures remain strict failures until the forward migrations resolve them; they are not a waiver. A mid-batch production repair failure can leave partial history; inventory it, stop, and obtain approval for an explicit continuation plan—never guess at rollback or proceed to push.
...
- A production backup captured after the last production write and restore-verified from that same backup is mandatory before the first hosted change. An older local backup cannot satisfy this gate; uncertainty stops the cutover.
...
- [ ] Owner separately authorized the exact 55-version production history-repair batch; immediate history reconciliation passed without an unresolved partial baseline.
- [ ] Owner's local database/schema backup and verified-restore attestation is recorded; the backup is confirmed to postdate the last production write and the restore check used that same backup. Any uncertainty caused a stop and a newly captured, restore-verified backup before the first hosted change. The controlled cutover start, separate history-repair/traffic-isolation/hosted-setting/rollback approvals and standing migration-apply authorization bound to the exact production candidate/file hashes are recorded; prior Realtime settings, affected-table publication state, and connected-client reports captured privately. Any backup workflow dispatch has its own authorization.
- [ ] Production thirteen-version `--include-all` dry run exact through the reviewed production-only runner; target binding rechecked immediately before apply.
...
## Bootstrap-preparation hold — 2026-09-20

Do not add a production command from the local preparation/rehearsal work to this runbook. The production non-dry-run block remains in force. Private preparation is retired fail closed; future private capability needs a separately reviewed protected installation/origin, trusted-owner ACL, read-sharing executable locks, and unforgeable bootstrap origin. Offline admission controls, cleanup/isolation collectors, exact dry-run and uncertain-failure proof, complete implementation gates, and a separately authorized owner decision remain prerequisites. The 2026-09-20 connector project inspection returned `INVALID_ARGUMENT`, but the subsequent authorized read-only Vercel CLI fallback completed the current pause/settings proof: the project and production target match the private pause record, automatic custom-domain assignment is disabled, and active cron definitions are present with count zero. This proof preserves the pause; it does not authorize a deployment, setting change, or reopening. [Evidence](evidence/production-bootstrap-preparation-2026-09-20.md).

## Local protected file-lease component — 2026-09-21

Protected Windows file-lease component at 7e54ae33d32d6cda791e3cd2143940573405e470. The 26 native Windows cases passed in the focused 53/53 run (5.19s); Linux CI classifies those 26 as Windows-kernel-only skips, never passes. Tested-implementation receipt SHA-256 34d28d1a8344fed29e9fc291397f5a85cd24f29a3a53b734f83604a353b05092. Production remains paused/no-go; Story 22.15 remains in-progress; Epic 23 remains on-hold. The component cannot be used to run a production command. The existing production non-dry-run block, separate cleanup/isolation/history-repair gates, and pause/reopening decision remain mandatory. Historical file-lease snapshot: trusted installation/origin, module closure and runner integration were still open at that stage. The later protected-toolchain and dry-run records below supersede those local implementation gaps; production-profile, traffic-isolation and target-write admission remain open. The 55 ledger rows are UNPROVED/unsigned, 13 executes are proposed, 48 orphan filters need separate approved cleanup, and seven dated strict production failures remain open.
...
## Local seven-case CLI verification — completed component

Use the [repository-owned local gate instructions](../../tests/support/production-cli-matrix-admission.md) only with a currently admitted, guard-owned synthetic PostgreSQL fixture and the reviewed source/tool pins. The required gate rejects missing admission and incomplete case execution. It never selects a hosted target. Each case uses a new retained database; no failed database is reset, repaired or continued. The exact thirteen immutable files are measured through pinned CLI 2.115.0, with fresh independent history/physical observers.

The explicit history-insert fault leaves the headcount constraint committed with no target history row; the separate implicit-column-absent derivative rolls back the current column addition. Timeout remains uncertain even after bounded server settling. These measured local outcomes are not a production continuation or repair recipe. The successful zero-filter derivative does not authorize deleting the 48 production orphan filters. Full success requires all thirteen local history versions and unchanged strict catalog 16/16.

Next collect fresh protected real-input/target/TLS and preservation evidence, prove isolation across application/jobs, Data API, Realtime, direct clients and database, and review target-write admission. Cleanup, history repair, settings, main, deployment and reopening retain their separate owner gates. Preserve the static production pause and API/job shutdown throughout.

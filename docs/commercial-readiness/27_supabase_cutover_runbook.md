...
> **Forward-bootstrap design proposal — 2026-09-19.** The current production procedure remains non-executable. A separate design proposes a hash-pinned materialized directory containing only the existing proposed 13 execute migrations, applied by normal reviewed CLI only after full isolation and separately approved cleanup. It must pass a local observed-profile fixture, pinned-CLI transaction/history behavior tests, and strict final proof before any lineage-aware ledger or repair; no raw SQL execution, manual history insert, or weaker legacy-policy reconstruction is allowed. [Design](30_production_forward_bootstrap_design.md).
...
Only after every verifier row passes, the fresh production inventory proves every listed version is materially represented, the complete 55-row ledger is signed, and the owner explicitly authorizes this exact **history-repair** batch may the operator run:
...
- [ ] Production fresh inventory/catalog proof passed for all 55 repair candidates; the remaining strict production groups are resolved without broadening the catalog contract.

...
Codex reviewed `56b4fb2c66ad85befbd31a3be43deb96b5f46f9e` and identified two valid verifier gaps: complete policy definitions could pass with table RLS disabled, and EXECUTE grantee comparisons omitted grant-option rights. Commit `2c8f0066bb4eda847f2ed6ae294a56dd87125652` requires RLS enabled on all eight tables in either complete policy profile and rejects non-owner function execution grants with delegation rights in both function-contract groups. Negative fixtures disable each protected table's RLS before and after reconciliation and add grant options for every applicable Story function role, both room functions, and staffing. The forward migration is unchanged. Catalog SQL SHA-256 is now `f35dd0d9a58e2b87dfb51387d301763c8a39d39f71af72b5e557765c7d10014e`.

Independent local review found no remaining actionable issue. Codex's repeat review of `2c8f006` completed at 2026-09-10T13:22:56Z with no new findings; the two inline findings belong to the earlier `56b4fb2` review. A final documentation-only head still requires its own remote checks and final review before merge approval.

| Review-fix gate | Result on `2c8f0066bb4eda847f2ed6ae294a56dd87125652` |
| --- | --- |
| Full Vitest | Exact `npx vitest run`: **3,451 passed, zero skips, zero failures, 322 files**, exit 0; 539.44 seconds report / 540.8180165 seconds wall; 13:16:45.4504105Z–13:25:46.2662654Z. Required local database/live-export proofs enabled with one worker; concurrency assertions remain enabled. |
| Full Playwright | Exact `npx playwright test`: **163 passed / 47 skipped / 0 failed / 0 errors**, exit 0; 1194.999401 seconds report / 1205.8919197 seconds wall; 13:26:27.7930473Z–13:46:33.6829666Z. XML SHA-256 `5a37a267454a33d40d9a9edf9b4a232fd3d39e487970f65c044e5bbe8c282477`. Every skip identity matches PR #96, retaining the same 9 authorization-dependent and 38 fixture/superseded classifications described above. |
| Clean migration chain | All 66 migrations plus seed on a fresh guard-owned `template0` fixture: 15/15 catalog checks, scoped fixture cleanup verified, 9.064 seconds. |
| Focused and PostgREST | Lead verifier/regression checks: 83/83, exit 0, 3.92 seconds; required ÖMC PostgREST contention test: 1/1, exit 0, 2.51 seconds. |
| TypeScript/lint | TypeScript exit 0; full lint exit 0, zero errors / 297 existing warnings. Application/build code and dependencies are unchanged, so the precisely scoped earlier preview/pause-build and audit evidence remains applicable. |
| GitHub/Vercel | Run `34481745929` on this exact implementation passed: unit 3,378 passed / 73 managed-environment skips (311 passing / 11 skipped files), 486.97 seconds; integration 827 passed / 73 managed-environment skips (93 passing / 11 skipped files), 149.57 seconds. These skips remain separate from the full local 3,451-test, zero-skip proof. Both Vercel checks passed. |

All guarded resources used for these local runs received successful Stop acknowledgments. The acknowledgments request asynchronous shutdown and are not claimed as proof of completed shutdown. The final CloseActor/List result and final documentation-head CI/Vercel/Reviewbot evidence will be attached to PR #98 before merge approval is requested; the actor remains available until all review-dependent work ends.

The `bc02f462baf67c6eb9cc8767b16098f0fcb6468a` documentation review identified two further documentation issues: old inventory sections still presented a 65-version/eight-execute plan and denied the completed Story 22.15 staging writes, while several current status fields still called the completed local verification open. The follow-up updates those sections, the linked overview/index pages, the specification's current plan, and all affected completion fields. Current records distinguish 65 applied staging versions from the 66-version repository candidate, permit zero staging repairs and only `20260910115024`, retain production's provisional 57-repair/nine-apply plan, and leave only final-head remote and owner/hosted gates open. This follow-up changes no tested implementation files.

### Retained CI failure and test-selector correction

GitHub run `34485109864` on `bc02f46` passed its unit step (3,378 passed / 73 managed skips, 383.57 seconds) but failed integration (826 passed / 73 managed skips / one failed, 116.11 seconds). The saved-filter UI test began typing before the panel completed its delayed initial focus, so the mock POST contained an incomplete name. Its global first-enabled “Spara filter” selector could also select the page button instead of the dialog action. A focused local diagnostic reproduced the same test failure (six passed / one failed, exit 1, 13.29 seconds), this time with the modal's pointer-events rejection. The first selector/focus patch still failed the lead focused run (six passed / one failed, exit 1, 5.17 seconds): the panel close button took focus. The final test-only correction waits for the panel initial-focus transition before opening the dialog, scopes the action to the named dialog, and verifies focus and the full typed value while preserving exact POST/success assertions. Three independent focused invocations passed 7/7 each; the lead focused run also passed 7/7, exit 0, 5.01 seconds. No sleeps, timeout extensions, disabled pointer checks, or application changes were introduced. The fresh full Vitest run on `269e3d5634c68e1bc502839678247833e1cccc54` passed 3,451 tests across 322 files with zero skips/failures, exit 0; 475.11 seconds report / 476.3469515 seconds wall, 2026-09-10T14:09:48.5316236Z–14:17:44.8762464Z. TypeScript passed and full lint passed with zero errors / 296 warnings. All local gates for the correction are closed. Application, migration, verifier and Playwright sources remain identical to the passing `2c8f006` browser run; that exact full Playwright result retains its stated commit scope.

## Production pause and remaining gates

Read-only Vercel inspections at 2026-09-10T12:04:21.493Z and 2026-09-10T13:48:23.619Z confirmed that the production domain still resolves to the recorded READY production pause deployment. The connector does not expose automatic production-domain assignment or active cron settings; their last independent dashboard proof is the historical 2026-09-09 disabled/zero result. Those settings must be freshly verified before any production-targeted action. No deployment, promotion, hosted setting change or reopening occurred during this preparation.

...
| Real custom columns on `employees` | Partner-specific columns added dynamically | configurable typed columns | Yes/Unknown | Unknown | Users/admin | Supabase PostgreSQL | Role permissions in app | Deleted column config; physical column drop not always automatic | Exports/backups | `column-config-repository.ts`, `add_custom_column_to_employees` | Requires governance over new fields |
| `column_config` | Column metadata and role permissions | column name, db column name, role_permissions | May include business metadata | Low/medium | Admin/users | Supabase PostgreSQL | Read broadly; admin/manage by role | Deleted/updated via API | Exports and administration | migrations, admin column routes | Diagnostic-route exposure was a pre-remediation risk; Story 22.1 removed the risky handlers |

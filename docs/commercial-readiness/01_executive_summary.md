...
PR #96 is merged to staging at `3724809065f9e4559802fdcd80e7e526172cc8fd`; main remains unchanged. The post-merge catalog diagnostic found only two absent defaults: `repayment_needed_omc` and `repayment_needed_pe3` each require `false`; all 22 represented columns match type/nullability and 20 defaults already match. The forward-only migration does not change existing values. The current release plan is 65 repository versions: one separately approved staging repair then seven applies, and provisional production 57 catalog-proven repairs then eight applies after fresh inventory. Its migration, manifest, verifier, and regression tests are pending, as are final-head CI/Vercel/review after push. The c27a9ee gates are historical evidence. The dependency audit remains 0 critical / 0 high / 1 accepted moderate (exit 1 solely for ExcelJS→UUID through 2026-09-30). Story 22.15 remains in-progress, Epic 23 remains on hold, and production stays paused. See [current evidence](evidence/staging-reconciliation-and-pause-2026-09-09.md), [repayment diagnostic](evidence/repayment-defaults-diagnostic-2026-09-10.md), and [pause safeguards](29_production_pause_release_safeguards.md).

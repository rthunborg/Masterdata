...
| Database RLS | RLS enabled on major tables in migrations; local `pg_policies` cross-check done; hosted production policy inventory reviewed via 2026-05-28 backup schema: 26 policies vs 22 migration-defined, with drifted names/definitions and empty remote migration history | migrations, `22_supabase_security_evidence_package.md`, `21_role_export_rls_test_evidence.md` | Partial; drift confirmed | Hosted policies were changed outside migrations; column-level permissions are app-layer | Reconcile hosted policies with migrations and baseline remote migration history (`R-023`, `R-010`) | High |
...
| Supabase SSO/MFA posture | Password auth via Supabase Auth; advisors confirm leaked-password protection is disabled; session/MFA settings not directly verified (owner review by 2026-09-30) | `22_supabase_security_evidence_package.md`, private platform-control evidence | Partial | Leaked-password protection off; detailed provider state held privately | Enable leaked-password protection; confirm Auth settings and enterprise SSO/MFA requirements | Medium |
...
| Backups | Nightly dumps to Supabase Storage with 14-day retention and partial staging restore; full restore drill of a production backup into a non-production target verified on 2026-06-11 | GitHub workflow, `evidence/restore-drill-2026-06-11.md` | Verified execution and restorability | Backups contain personal data; auth users are outside logical backup scope; one nightly run (2026-06-05) is missing from the bucket | Add backup-failure alerting; keep periodic restore drills; restrict bucket access | High |
| Supabase physical backups | PITR not enabled (paid feature, current plan kept per NFR1); risk-accepted with operations owner, review 2026-09-30; GitHub logical backups are the verified mechanism | `22_supabase_security_evidence_package.md`, `evidence/restore-drill-2026-06-11.md` | Risk-accepted with owner/date | RPO limited to nightly logical backups (~24h nominal; the unnoticed 2026-06-05 backup failure shows the effective recovery point is not assured without failure alerting, Story 22.12) | Revisit PITR if a contract requires sub-24h RPO | High |
...
8. Managed-platform backup/PITR posture is resolved for the working-pilot scope: PITR remains disabled (paid feature, risk-accepted by operations owner, review 2026-09-30) and the GitHub logical backup mechanism passed a full restore drill on 2026-06-11 (`evidence/restore-drill-2026-06-11.md`). Follow-ups: backup-failure alerting (2026-06-05 run missing) and auth-user provisioning in recovery planning.
...
- Backup restore test is done (full restore drill 2026-06-11, `evidence/restore-drill-2026-06-11.md`); document backup encryption/storage/access controls and add backup-failure alerting.

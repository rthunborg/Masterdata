...
- `supabase/migrations/` contains **63 versioned migration files** as of Story 22.15. The original `20250113000000` room-assignment migration is restored byte-for-byte and the new forward active-authorization migration is `20260831200026`. `supabase/migration-baseline-manifest.json` classifies all 63 exactly once: 57 repair-after-catalog-proof versions and six forward applies. No hosted catalog proof is claimed complete.

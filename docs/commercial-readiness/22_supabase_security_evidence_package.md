...
- Historical Story 22.15 snapshot: `supabase/migrations/` contained 63 versioned migration files, with 57 repair-after-catalog-proof versions and six forward applies. The subsequent 2026-09-09 preparation added `20260909115242`, producing the historical 64-version / six-staging-apply / seven-production-apply plan. The current proposed 2026-09-10 plan adds execute-only `20260910094517`: 65 versions, staging one repair plus seven applies, and provisional production 57 repairs plus eight applies. No hosted catalog proof is claimed complete.
...
- **Story 22.10 hosted-staging observation (2026-06-14):** `migration list` showed **57 migrations local↔remote in sync**. That dated fact remains historical. The current proposed Story 22.15 sequence requires a fresh inventory, one explicit history repair (`20250113000000`) only after catalog proof, then seven staging applies through `20260910094517`; production requires fresh inventory, proof, and a signed ledger for all 57 repair versions before eight forward applies. Both plans retain the reviewed target-binding, `--dry-run --skip-vault`, immutable commit, backup/go-live, isolation, and separate owner gates described in the cutover runbook. No hosted action is authorized.

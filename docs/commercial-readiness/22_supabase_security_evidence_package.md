...
Updated: 2026-09-10 — dependency remediation, focused gates, and exact-head evidence recorded; fresh full local and hosted gates remain open

Story: 22.8, with Story 22.10, Story 22.13, and Story 22.15 addenda

> **Current Story 22.15 amendment:** the merged staging baseline is `8c82bd8f4cc3c5076b2b6a37f4ced209bd8cba1c` (main unchanged; no intervening commits). The revised manifest has 64 versions: staging requires separate catalog proof, one repair, then six applies; production remains provisional at 57 repairs plus seven applies after fresh inventory. Read-only staging evidence found zero saved-filter rows requiring cleanup and captured redacted repayment/permission aggregates. The 2026-09-10 narrow dependency patch returns audit to 0 critical/0 high/1 accepted UUID moderate, exit 1 solely for the existing acceptance through 2026-09-30. Focused evidence is 197/197, zero skips; fresh full local gates remain pending. No hosted mutation has occurred. Earlier dated 63-version/five- and six-apply statements are historical; this amendment governs current release preparation.
...
- Historical Story 22.15 snapshot: `supabase/migrations/` contained 63 versioned migration files, with 57 repair-after-catalog-proof versions and six forward applies. Current preparation adds execute-only `20260909115242`, so the manifest now classifies 64 versions: staging one repair plus six applies, and provisional production 57 repairs plus seven applies after fresh inventory. No hosted catalog proof is claimed complete.
...
- **Story 22.10 hosted-staging observation (2026-06-14):** `migration list` showed **57 migrations local↔remote in sync**. Story 22.15 requires a fresh inventory, one explicit history repair (`20250113000000`) only after its catalog proof, then six forward staging applies through `20260909115242`, each under its separate gate. Production requires fresh inventory, proof and a signed ledger for all 57 repair versions before seven forward applies. Both plans require a separately supplied intended-project reference matching the CLI link and an explicitly selected database connection mode, a hash/version-approved absolute `psql`, an explicit hash-approved CA with `verify-full`, a machine fail-closed read-only catalog wrapper, exact lists, `--dry-run --skip-vault`, immutable commit, backup/go-live gates, and owner authorization. Direct mode binds the project-specific hostname. The approved IPv4-only alternative is Shared Supavisor session mode on port `5432`, bound independently through the project reference encoded in the username and a separately approved exact pooler hostname; transaction mode on `6543` is prohibited. Production additionally requires a private affected-table Realtime publication/current-client inventory, separately authorized temporary Realtime-service disablement with existing/reconnect probes, and proven application/Data API/direct-DB isolation across repair/apply/post-verification because its seven migration files can commit partially. Keep the existing production pause; use staging for application smoke. Production deployment and reopening require separate approval. The exact prior Realtime/publication/connection state is restored and verified only under its separate gate (`27_supabase_cutover_runbook.md`, `R-025`).
- **Production staffing evidence contradiction (2026-09-01 audit):** the dated restore-drill record names `staffing_needs.target_headcount`, while the repository/application contract is `headcount_need` with an exact `0..9999` constraint. The deleted drill source cannot resolve the discrepancy. Fresh read-only production catalog proof is mandatory before staffing-history repair; a hosted `target_headcount` shape requires an approved forward reconciliation and stop, not a repair-as-applied entry.
- **PR #95 actual-connection remediation (2026-09-07):** review found that proving `SUPABASE_DB_URL` did not constrain the pinned CLI's independent native `--linked` target/fallback path. Every remote database command now requires wrapper-only `--reviewed-target`; the wrapper repeats target and CA proof, rejects native selectors, strips ambient PostgreSQL/TLS variables, and invokes CLI `2.115.0` with a generic passwordless `--db-url` plus only the validated connection fields and `verify-full`/reviewed CA in a minimal environment. Private URL, project identity, hostname, and password remain out of process arguments.
- **PR #95 command-grammar remediation (2026-09-07):** Codex Reviewbot found that leading Cobra persistent options could conceal a database command from the wrapper and that `-p` could override the validated password while exposing it in process arguments. The wrapper now rejects every leading root option, rejects embedded persistent flags by accepting only exact runbook database shapes, and rejects `-p` plus attached short-password values without echoing them. Only the standalone version probe and exact `db push --help` discovery command remain approved option-bearing exceptions outside reviewed database calls.
- **PR #95 environment-specific repair allowlist (2026-09-07):** a subsequent Codex Reviewbot P1 found that any syntactically valid 14-digit migration could still reach `migration repair --status applied`. Repair now requires a wrapper-only `staging|production` assertion matching the reviewed private target record and accepts only that environment's `repair-after-catalog-proof` set from the committed manifest. Forward-only, arbitrary, cross-environment, missing-environment, and mismatched-record inputs fail before target proof or spawn; both wrapper-only markers are removed before the pinned CLI is invoked.
...
Current local verification record: Earlier candidate full results are historical because they predate the guard corrections. The fresh trusted hook resolves ACTOR_CLOSING; renewed full local Vitest and Playwright gates remain pending. Focused evidence is 197/197 across 14 files in 37.85 seconds with zero skips; TypeScript and lint pass, and the required observed-state fixture and live-export cases are enabled. The narrow patch returns audit to 0 critical/0 high/1 accepted UUID moderate (exit 1 solely for the existing acceptance through 2026-09-30). Exact-head GitHub/Vercel checks are green on `7826a339414fccf6694798ef039a2e1e4403a8b6`; Reviewbot has no findings and all eight inline threads are resolved. Story 22.15 stays in-progress; Epic 23 stays on hold. Hosted repair/apply, staging/main merges, production deployment/settings and reopening remain separately owner-gated. See [dated evidence](evidence/staging-reconciliation-and-pause-2026-09-09.md).

...
- **Story 22.10 hosted-staging observation (2026-06-14):** `migration list` showed **57 migrations local↔remote in sync**. Story 22.15 requires a fresh inventory, one explicit history repair (`20250113000000`) only after its catalog proof, then five forward staging applies. Production requires fresh proof and a signed ledger for all 57 repair versions before six forward applies. Both plans require a separately supplied intended-project reference matching the CLI link and an explicitly selected database connection mode, a hash/version-approved absolute `psql`, an explicit hash-approved CA with `verify-full`, a machine fail-closed read-only catalog wrapper, exact lists, `--dry-run --skip-vault`, immutable commit, backup/go-live gates, and owner authorization. Direct mode binds the project-specific hostname. The approved IPv4-only alternative is Shared Supavisor session mode on port `5432`, bound independently through the project reference encoded in the username and a separately approved exact pooler hostname; transaction mode on `6543` is prohibited. Production additionally requires a private affected-table Realtime publication/current-client inventory, separately authorized temporary Realtime-service disablement with existing/reconnect probes, and proven application/Data API/direct-DB isolation across repair/apply/post-verification, exact-candidate deployment, and smoke because its six migration files can commit partially. The exact prior Realtime/publication/connection state is restored and verified only after smoke (`27_supabase_cutover_runbook.md`, `R-025`).

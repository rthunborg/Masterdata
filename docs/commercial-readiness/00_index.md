...
| `26_environment_reconciliation_inventory.md` | Security, IT, reviewers | Dated Story 22.10 three-way drift inventory plus Story 22.15's 65-version manifest partition, catalog-proof requirements, exact staging/production repair/apply deltas, and 17-policy target. Story 22.10 staging writes remain historical; the Story 22.15 hosted delta is unapplied. Cutover steps: `docs/commercial-readiness/27_supabase_cutover_runbook.md`; change policy: `docs/commercial-readiness/28_migrations_only_change_policy.md` |
| `27_supabase_cutover_runbook.md` | Security, IT, operations | Manifest-governed sequence: fresh catalog proof, signed 57-row production ledger, exact explicit repairs, `db push --dry-run --skip-vault`, seven staging/eight production applies, separate backup/maintenance/database-apply authorization while preserving the pause, and post-apply verification. No Story 22.15 hosted action has executed. |
...
| [Current reconciliation/pause evidence](evidence/staging-reconciliation-and-pause-2026-09-09.md) | All reviewers | Redacted preflight, exact revisions, local verification attempts and remaining gates |
...
- Supabase Auth session lifetime and MFA settings (dashboard-only). Preserve the dated database states: Story 22.10 hosted staging was verified at 57 migrations/19 policies on 2026-06-14; the Story 22.15 repository now plans 65 versions and a 17-policy target. After fresh catalog proof, staging repairs only `20250113000000` and applies seven versions; production requires fresh inventory, proof and separate approval for the 57-version repair list and eight applies. Production remains paused. Leaked-password protection + CAPTCHA remain Epic 23 Story 23.4.
...
- Story 22.15 supersedes the Story 22.13 hosted release plan with explicit catalog proof, manifest-approved repair lists, `--dry-run --skip-vault`, and six pending staging applies through `20260909115242`. Hosted writes still require owner approval; local/static evidence is not hosted proof. See `17_blocker_remediation_tracker.md#tracker`.

...
Current Story 22.15 state (2026-09-10): separately authorized staging repair/seven-apply work completed at 65/65. The repository target is 66 versions and 17 policies; staging repair set is `[]` and only `20260910115024` remains pending. Apply requires the separately approved staging merge, fresh read-only proof, and separate migration approval. Production remains provisional after fresh inventory at 57 repairs plus nine executes. Exact 2c8 local gates passed: Vitest 3,451/3,451 with zero skips and Playwright 163 passed / 47 classified skips / 0 failed / 0 errors; 2c8 CI, Vercel, and Reviewbot are clean. Final test-only selector `269e3d5634c68e1bc502839678247833e1cccc54` passed full Vitest 3,451/3,451 zero skips, TypeScript, and lint 0 errors/296 warnings; application/migration/verifier/Playwright sources are unchanged from 2c8, so Playwright remains 2c8-scope evidence and was not rerun. Production remains unchanged and paused, Story 22.15 remains in-progress, and Epic 23 remains on hold. The future documentation-only head still needs its own CI, Vercel, and Reviewbot verification after push.
...
| `26_environment_reconciliation_inventory.md` | Security, IT, reviewers | Dated Story 22.10 inventory plus current Story 22.15 66-version/17-policy target. Staging reached 65/65 under separate authorization; only `20260910115024` remains as the current staging execute after reviewed merge and fresh read-only proof. Production stays provisional at 57 repairs plus nine executes. |
| `27_supabase_cutover_runbook.md` | Security, IT, operations | Manifest-governed current sequence: reviewed merge and fresh read-only proof, then separately authorized staging execute `20260910115024`; production remains fresh-inventory/ledger-gated at 57 repairs plus nine executes, with pause preserved and post-apply verification required. |
...
- Supabase Auth session lifetime and MFA settings (dashboard-only). Preserve the dated 2026-06-14 57-migration/19-policy snapshot as historical. Current Story 22.15 staging is 65/65; repository target is 66 versions/17 policies with repair set empty and only `20260910115024` pending reviewed merge, fresh proof, and separate execute approval. Production remains provisional at 57 repairs plus nine executes and paused. Leaked-password protection + CAPTCHA remain Epic 23 Story 23.4.
...
- Story 22.15 supersedes the historical Story 22.13 hosted release plan. Staging is now 65/65; only `20260910115024` remains pending reviewed merge, fresh read-only proof, and separate execute approval. Hosted writes still require owner approval; local/static evidence is not hosted proof. See `17_blocker_remediation_tracker.md#tracker`.

...
PR #96 is merged to staging at `3724809065f9e4559802fdcd80e7e526172cc8fd`; main remains `822350986f4c023948a7bbf490ddffc371185c4a`. A post-merge read-only catalog diagnostic found only the absent `false` defaults on `repayment_needed_omc` and `repayment_needed_pe3`: all 22 represented columns match type/nullability and the other 20 defaults. The forward-only candidate adds `20260910094517_reconcile_repayment_defaults.sql` without changing existing rows, so the current plan is 65 repository versions: one staging repair plus seven exact applies, and provisional production 57 catalog-proven repairs plus eight applies after fresh inventory. New migration/verifier/manifest regression tests and final-head CI/Vercel/review remain pending. The earlier c27a9ee local/remote gates and parent-SHA review correction are historical evidence only. The dependency audit remains 0 critical/0 high/1 accepted UUID moderate, with exit 1 solely for the existing acceptance through 2026-09-30. Story 22.15 remains in-progress, Epic 23 remains on hold, and production remains paused. Hosted repair/apply, deployment, settings changes, and reopening require separate owner decisions. See [current evidence](evidence/staging-reconciliation-and-pause-2026-09-09.md), [repayment diagnostic](evidence/repayment-defaults-diagnostic-2026-09-10.md), and [pause safeguards](29_production_pause_release_safeguards.md).

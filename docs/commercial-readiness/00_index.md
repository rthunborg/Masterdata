...
| `26_environment_reconciliation_inventory.md` | Security, IT, reviewers | Dated Story 22.10 three-way drift inventory plus Story 22.15's 63-version manifest partition, catalog-proof requirements, exact staging/production repair/apply deltas, and 17-policy target. Story 22.10 staging writes remain historical; the Story 22.15 hosted delta is unapplied. Cutover steps: `docs/commercial-readiness/27_supabase_cutover_runbook.md`; change policy: `docs/commercial-readiness/28_migrations_only_change_policy.md` |
| `27_supabase_cutover_runbook.md` | Security, IT, operations | Manifest-governed sequence: fresh catalog proof, signed 57-row production ledger, exact explicit repairs, `db push --dry-run --skip-vault`, five staging/six production applies, backup/maintenance/go-live authorization, and post-apply verification. No Story 22.15 hosted action has executed. |
...
- Story 22.15 refreshed `pnpm audit --prod --json`: the candidate had regressed to 28 advisories (15 high). The remediated lockfile reports 0 critical, 0 high, 1 moderate, and 0 low; the sole ExcelJS→UUID residual is time-bounded through 2026-09-30. Current output is captured in `evidence/dependency-audit-2026-08-31.md`.
...
- Story 22.15 repository state (2026-09-01): 63 migration versions are explicitly partitioned by `supabase/migration-baseline-manifest.json` into 57 versions repairable only after environment-specific catalog proof and six forward applies. The original `20250113000000` room-assignment migration is restored byte-for-byte, inactive JWTs no longer receive an application role, saved filters require an active app user, and app-user deletion is atomic before separate Auth cleanup. A clean local reset applied all 63 migrations; live database 11/11, Story 22.14 PostgREST 1/1, live export 5/5, full Vitest 3,342/3,342 with zero skips, exact full Playwright 163 passed / 47 classified skips / 0 failed, type-check, zero-error lint, production build, and dependency threshold passed. The 47 E2E skips are not passing evidence: 9 require explicitly authorized non-production notification capture and 38 are obsolete/superseded or deterministic-fixture coverage debt. Historical Story 22.10 staging changes occurred on 2026-06-14, but no Story 22.15 hosted delta has been applied. The authoritative branch observations were `origin/staging` `bcb1a0e5bed3d06b9b7582320f491964ffc5a0b9` and `origin/main` `822350986f4c023948a7bbf490ddffc371185c4a`; neither is the still-required final reviewed remediation SHA.

## Needs Manual Review

- Supabase Auth session lifetime and MFA settings (dashboard-only). Preserve the dated database states: Story 22.10 hosted staging was verified at 57 migrations/19 policies on 2026-06-14; the Story 22.15 repository has 63 versions and a 17-policy target. After fresh catalog proof, staging repairs only `20250113000000` and applies five versions; production repairs the explicit 57-version historical list and applies six. Leaked-password protection + CAPTCHA remain Epic 23 Story 23.4.
...
- Story 22.15 supersedes the Story 22.13 hosted release plan with explicit catalog proof, manifest-approved repair lists, `--dry-run --skip-vault`, and five pending staging applies. Hosted writes still require owner approval; local/static evidence is not hosted proof. See `17_blocker_remediation_tracker.md#tracker`.

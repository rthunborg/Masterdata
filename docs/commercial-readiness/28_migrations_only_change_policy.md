...
Status: active from Story 22.10 (2026-06-14)
...
1. **Author in migrations.** Every hosted schema/policy/function/grant change is a new timestamped file in `supabase/migrations/` (`YYYYMMDDHHMMSS_<desc>.sql`), idempotent/guarded where practical (`DROP POLICY IF EXISTS`, `CREATE OR REPLACE FUNCTION`, `to_regprocedure` guards, `ADD/DROP COLUMN IF EXISTS`).
...
3. **Keep the remote history in sync.** After Story 22.10's baseline, `supabase migration list --linked` must show local↔remote in sync for staging and production. Never `supabase db push` net-new schema against an environment whose remote history is not baselined.
4. **Staging before production.** Apply and verify on staging, then production — never the reverse. Production applies happen only at an owner-approved cutover (see `27_supabase_cutover_runbook.md`).
5. **No dashboard schema edits.** Auth/platform settings that are genuinely dashboard-only (e.g. leaked-password protection, session/MFA config) are recorded as dated "changed on `<date>` by `<operator role>`" entries in the evidence index, since they are not expressible as SQL migrations.
6. **Local-stack parity is not hosted schema.** `supabase/seed.sql` grants the public-schema API-role table privileges that the hosted platform provisions automatically but the local CLI stack does not. It runs only on `supabase db reset`/`start` and is never applied to hosted environments — it is local parity, not a hosted change.

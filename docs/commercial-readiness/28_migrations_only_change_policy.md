...
Current amendment: PR #96 is merged at staging `3724809065f9e4559802fdcd80e7e526172cc8fd`; main is unchanged. The post-merge read-only catalog diagnosis found only two absent employee-column defaults, with all 22 represented columns matching type/nullability and 20 defaults. The new execute-only `20260910094517` sets `false` defaults for `repayment_needed_omc` and `repayment_needed_pe3` without altering any existing values. The manifest now plans one separately authorized staging repair then seven exact applies; production is provisional at 57 catalog-proven repairs then eight applies after fresh inventory. New local migration/verifier/manifest regression tests and final-head CI/Vercel/review are pending. Earlier c27a9ee gates remain historical evidence. Story 22.15 remains in-progress, Epic 23 remains on hold, production remains paused, and no hosted mutation is authorized by this policy.

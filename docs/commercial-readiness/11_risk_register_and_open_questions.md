...
| R-007 | Full restore drill (verified 2026-06-11; follow-ups closed 2026-06-16) | Operations | Full restore drill of the 2026-05-28 production backup into the local non-production stack succeeded on 2026-06-11 with 7 of 8 validation checks passed (one not applicable: no serial/identity columns to restore). Both residual follow-ups are now closed by Story 22.12 (2026-06-16): backup-failure alerting was added (the 2026-06-05 nightly run had been missing from the bucket undetected), and auth-user provisioning is documented in the recovery runbook (auth schema outside logical backup scope) | Low | High | Low | Verified nightly backup pipeline plus documented full restore drill; nightly workflow now alerts on any failure and retries the CLI setup once | Repeat restore drills periodically | Operations owner | Resolved in Story 22.8; follow-ups closed in Story 22.12 (2026-06-16): `if: failure()` GitHub-issue alerting + one-shot CLI-setup retry (pinned version) added, auth-user provisioning documented, and the staging-refresh `users` exclusion deliberately kept for auth-link integrity | `evidence/restore-drill-2026-06-11.md`, `evidence/backup-failure-alerting-2026-06-16.md`, backup workflow, `scripts/notify-backup-failure.mjs` |
...
| R-019 | Supabase physical backup/PITR posture | Operations | PITR is not enabled (paid feature; current plan kept per NFR1); GitHub logical backups are the verified mechanism and passed a full restore drill on 2026-06-11; nominal RPO follows the nightly schedule (~24h); the 2026-06-05 run had failed undetected for six days, but Story 22.12 (2026-06-16) added backup-failure alerting (`if: failure()` GitHub-issue step) plus a one-shot CLI-setup retry with a pinned version, so a silent backup gap can no longer go undetected | Medium | High | High | Verified nightly logical backup pipeline plus full restore drill; nightly workflow now alerts on any failure | Revisit PITR/platform backups if a contract requires sub-24h RPO | Operations owner — Rasmus Thunborg | Risk-accepted 2026-06-11 (owner named 2026-06-12); review 2026-09-30; backup-failure alerting added 2026-06-16 (Story 22.12) | `22_supabase_security_evidence_package.md`, `evidence/restore-drill-2026-06-11.md`, `evidence/backup-failure-alerting-2026-06-16.md` |
...
- Full backup restore test is done (2026-06-11, `evidence/restore-drill-2026-06-11.md`); backup-failure alerting and a one-shot CLI-setup retry were added in Story 22.12 (2026-06-16). Keep periodic drills.

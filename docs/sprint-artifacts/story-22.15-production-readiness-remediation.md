...
- `git diff --check`: exit `0`; repository migration count is 63.
...
- Database/migration safety: restored room migration, forward Story 22.15 migration, 63-version manifest, read-only catalog verifier, helper/live/static tests.
- Dependencies: `package.json`, `pnpm-workspace.yaml`, `pnpm-lock.yaml`, Nodemailer compatibility test, audit evidence/register.
- Readiness/status: commercial-readiness index, security/auth/role/dependency/evidence/risk/reconciliation/runbook/policy surfaces plus all Story 22.15 sprint/BMAD status carriers.

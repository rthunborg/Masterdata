...
- **Story Points:** 5
- **Dependencies:** `22.6`
- **BMAD implementation artifact:** `_bmad-output/implementation-artifacts/22-7-add-role-export-and-rls-evidence-tests.md`
...
- [x] [Review][Patch] Redact or move detailed operational/security evidence out of public repository docs — Commercial-readiness docs added or expanded in this story set include production domain references, Supabase project refs/regions, Vercel deployment IDs, disabled security-feature observations, database network-restriction posture, and secret names while also documenting that the repository is public. User chose to redact or move private during review resolution.
- [x] [Review][Patch] Preserve recruiter/admin_limited custom-column values in `/api/employees` list responses [src/app/api/employees/route.ts:56]
- [x] [Review][Patch] Replace the duplicated external-role response-shaping list with the canonical role helper [src/app/api/employees/route.ts:57]
- [x] [Review][Patch] Return a 400 export validation response for malformed JSON before generic error handling [src/app/api/employees/export/route.ts:36]
- [x] [Review][Patch] Broaden AC1 field/editability tests so all eight roles are verified against the field matrix instead of spot-checked fixtures [tests/unit/epic-22/story-22.7/role-field-permissions.test.ts:51]
- [x] [Review][Patch] Broaden AC2 API/export isolation tests to cover OMC, Toplux, Crewing, and custom export fields before claiming external-role coverage [tests/unit/epic-22/story-22.7/api-field-isolation.test.ts:130]
- [x] [Review][Patch] Align RLS evidence with claimed external-role coverage by adding OMC, Payroll, Toplux, and all non-Crewing staffing denial cases [tests/integration/epic-22/story-22.7/supabase-rls-evidence.test.ts:165]
- [x] [Review][Patch] Make the RLS evidence setup claim match the implementation or switch setup/cleanup to service-role-backed setup as specified [tests/integration/epic-22/story-22.7/supabase-rls-evidence.test.ts:42]
- [x] [Review][Patch] Seed or upsert the staffing_needs row inside the Story 22.7 RLS transaction so the test is hermetic [tests/integration/epic-22/story-22.7/supabase-rls-evidence.test.ts:215]
- [x] [Review][Defer] Pre-existing arbitrary user-activity update risk remains documented in the API matrix [docs/commercial-readiness/19_api_auth_matrix.md] — deferred, pre-existing
...
- 2026-06-08: Expanded Story 22.7 into dev-agent-ready context with role/export/RLS evidence tasks, guardrails, known risks, sources, and mandatory test gates.

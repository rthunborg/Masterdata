...
**As an** HR Admin,  
**I want** to maintain a reference calendar of important operational dates,  
**so that** I can track and manage key Stena and ÖMC dates throughout the year.

---

## Acceptance Criteria

1. Navigation includes "Important Dates" link accessible only to HR Admin role (external parties do not need access to operational dates)
...
8. HR Admin can manage (create/update/delete) important dates with full edit access
9. Table is sortable by week number and filterable by category
10. RLS policy enforces that only hr_admin role can read/create/update/delete important_dates (external parties have no access)
...
14. Route protection ensures external parties are redirected or receive 403 when attempting to access /important-dates
...
| 2025-11-02 | 1.2     | Access restriction updated: Important Dates now HR Admin only (AC 1, 8, 10, 14 modified) - See Story 7.6 for implementation of access control changes                     | Bob (SM)    |
